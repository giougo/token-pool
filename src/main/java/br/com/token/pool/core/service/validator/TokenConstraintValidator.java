...
public interface TokenConstraintValidator extends Function<Token, TokenValidationResult> {
    ...
    TokenConstraintValidator IS_EXPIRATION_AMOUNT_PRESENT = token -> token.getExpiresInMilliseconds() == 0 ? SUCCESS : ABSENT_EXPIRATION_TIME;
    
    TokenConstraintValidator IS_EXPIRATION_TIMESTAMP_PRESENT = token -> token.getExpirationTimestamp() != null ? SUCCESS : ABSENT_EXPIRATION_TIMESTAMP;
    ...
    TokenConstraintValidator IS_TOKEN_EXPIRED = (token) -> {
		...
		Long tokenExpirationTimestamp = token.getExpirationTimestamp();
